 ...
 * Based on the 2code project's EnhancedStreamingMessageParser
 */
...
export class EnhancedStreamingMessageParser {
  ...
  constructor(private _callbacks: ParserCallbacks = {}) {}
  ...
  parse(messageId: string, input: string): string {
    ...
    for (const pattern of patterns) {
      enhanced = enhanced.replace(pattern.regex, (match, ...args) => {
        ...
        if (this._isShellCommand(content, language)) {
          ...
          console.log(`[EnhancedParser] Auto-wrapped code block as shell command instead of file`);
        ...
        console.log(`[EnhancedParser] Auto-wrapped code block as file: ${filePath}`);
    ...
    enhanced = enhanced.replace(fileOperationPattern, (match, filePath, content) => {
      ...
      const wrapped = this._wrapInArtifact(artifactId, filePath, content);
      console.log(`[EnhancedParser] Auto-wrapped file operation: ${filePath}`);
  ...
  private _wrapInArtifact(artifactId: string, filePath: string, content: string): string {
    ...
    // Trigger callbacks for artifact and action
    this._callbacks.onArtifactOpen?.({
      messageId: '',
      artifactId,
      title,
      type: 'bundled',
    ...
    this._callbacks.onActionOpen?.({
      artifactId,
      messageId: '',
      actionId: `${artifactId}-action`,
      action: {
        type: 'file',
        filePath,
        content,
    ...
    this._callbacks.onArtifactClose?.({
      messageId: '',
      artifactId,
      title,
      type: 'bundled',
    ...
    // Return the original content (we handle it via callbacks)
    return content;
  }
  ...
  private _wrapInShellAction(content: string, messageId: string): string {
    ...
    // Trigger callbacks for shell action
    this._callbacks.onArtifactOpen?.({
      messageId: '',
      artifactId,
      title: 'Shell Command',
  ...
  private _detectAndWrapShellCommands(_messageId: string, input: string, processed: Set<string>): string {
    ...
    return input.replace(shellCommandPattern, (match, language, content) => {
      ...
      if (this._isShellCommand(content, language)) {
        processed.add(blockHash);
        console.log(`[EnhancedParser] Auto-wrapped shell code block as command: ${language}`);
